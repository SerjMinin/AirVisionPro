...
let offsetSteps = 0;          // 0 = настоящее; 1 = на 1 окно назад; и т.д.
let chart = null;

const RANGES = {
  "24h":   { sec: 24*3600,        ticks: 24  },
  "week":  { sec: 7*24*3600,      ticks: 7   },
  "month": { sec: 30*24*3600,     ticks: 30  },
  "year":  { sec: 365*24*3600,    ticks: 12  }
};
const RANGE_LABELS = { "24h":"24ч", "week":"Неделя", "month":"Месяц", "year":"Год" };
...
function buildRangeBar() {
  ...
  ["24h","week","month","year"].forEach(code => {
    ...
    b.textContent = RANGE_LABELS[code];       // БЕЗ перевода
...
/* ---- окно времени с учётом листания ---- */
...
function smoothJ305(points) {
  ...
  return points.map((pt, i) => {
    ...
    return { x: pt.x, y: spike ? pt.val : avg };
  });
}
...
async function renderParam(key) {
  ...
  // Спец-режим: компас направления ветра
  if (p.type === "compass") { await renderCompass(p); return; }

  showCanvasGraph();
  const { from, to, span } = windowRange();
  ...
  // набор ключей: для PM — несколько линий, иначе — один key
  ...
  for (const s of sources) {
    for (const ld of lineDefs) {
      ...
      // группировка по провайдеру/источнику (датчик, Open-Meteo, OWM)
      const groups = {};
  ...
  for (let i = 0; i <= ticks; i++) {
    const unix = from + i * step;
    xLabels.push(fmtTick(unix, currentRange));
  }
  ...
  chart = new Chart(document.getElementById("chart"), {
    ...
    options: {
      ...
      scales: {
        x: {
          type: "linear", min: 0, max: ticks,
          grid: { color: gridColor },
          ticks: {
            color: tickColor, stepSize: 1, autoSkip: false, maxRotation: 0,
...
function fmtTick(unix, range) {
  ...
  if (range === "24h")  return p(d.getHours());                       // часы 0..23
  if (range === "week") return p(d.getDate()) + "." + p(d.getMonth()+1);
  if (range === "month")return p(d.getDate());
  return p(d.getMonth()+1);                                            // год → месяцы 1..12
}

/* ============================================================
   КОМПАС (роза ветров) — рисуется на canvas вручную
   ============================================================ */
...
async function renderCompass(p) {
  ...
  // 8 секторов, частота попаданий
  const bins = new Array(8).fill(0);
  rows.forEach(r => {
    let deg = ((Number(r.val) % 360) + 360) % 360;
    const idx = Math.round(deg / 45) % 8;   // 0=С,1=СВ,...
    bins[idx]++;
  });
  const total = rows.length || 1;
  const freq = bins.map(b => b / total);            // доля 0..1
...
function drawCompass(freq, currentDeg) {
  ...
  const size = Math.min(cv.parentElement.clientWidth, cv.parentElement.clientHeight) - 20;
  ...
  // лучи и подписи румбов
  const labels = t("COMPASS"); // [С,СВ,В,ЮВ,Ю,ЮЗ,З,СЗ]
  for (let i=0;i<8;i++){
    const ang = (i*45 - 90) * Math.PI/180; // 0=С сверху
...
async function startDashboard() {
  ...
  window.addEventListener("resize", () => { if (PARAMS.find(x=>x.key===currentKey)?.type==="compass") renderParam(currentKey); });
}